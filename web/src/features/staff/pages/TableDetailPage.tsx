...
import { useTableQuery, useOpenTableOrderMutation } from '../../../hooks/useTables'
...
import {
  ChevronLeft, Plus, Minus, Trash2,
  Receipt, CreditCard, Sparkles, ChefHat, CheckCircle2, AlertCircle, ShoppingBag, X, Search
} from 'lucide-react'
...
export default function TableDetailPage() {
  ...
  const [localCart, setLocalCart] = useState<Record<number, { id: number; nameEn: string; nameMy: string; price: number; quantity: number; note: string }>>({})
  ...
  const handleUpdateItemStatus = async (itemId: number, nextStatus: 'pending' | 'preparing' | 'served') => {
    if (!activeOrder) return
    try {
      await updateItemMutation.mutateAsync({
        orderId: activeOrder.id,
  ...
  const handleAddItemsSubmit = async () => {
    ...
    try {
      ...
      await addItemsMutation.mutateAsync({ orderId: activeOrder.id, items: payload })
      toast.success(t('common:success'))
      setLocalCart({})
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 py-4 border-b border-border">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t('staff:capacitySeats', { count: table.capacity, defaultValue: `Capacity: ${table.capacity} Seats` })} &bull;{' '}
            {isOccupied
              ? t('staff:itemsOrdered', { count: activeOrder.items?.length || 0, defaultValue: `${activeOrder.items?.length || 0} items ordered` })
        ...
        <Card className="glass border-border rounded-3xl p-12 text-center max-w-md mx-auto space-y-6 bg-card">
          <CardContent className="space-y-6 p-0">
            ...
            <div className="space-y-2">
              <h2 className="text-lg font-black text-foreground">{t('staff:tableAvailable')}</h2>
              <p className="text-xs text-muted-foreground">
                {t('staff:tableAvailableDesc', { defaultValue: 'This table is currently clean and vacant. Tap open order to start a new dining session.' })}
              </p>
            </div>
            <Button
          ...
          <div className="space-y-2.5">
                ...
                >
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex items-center gap-2.5">
                      ...
                      {item.status === 'pending' && (
                        <Badge variant="outline" className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border-amber-500/25 h-auto">
                          {t('order:pending')}
                        </Badge>
                      )}
                      {item.status === 'preparing' && (
                        <Badge variant="outline" className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border-blue-500/25 h-auto">
                          {t('order:preparing')}
                        </Badge>
                      )}
                      {item.status === 'served' && (
                        <Badge variant="outline" className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border-emerald-500/25 h-auto">
                          {t('order:served')}
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 border-border pt-3.5 sm:pt-0">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <span>{t('order:price', { defaultValue: 'Price' })}:</span>
                      <CurrencyDisplay amount={item.unit_price * item.quantity} className="font-bold text-foreground" />
                    </div>
            ...
            {(!activeOrder.items || activeOrder.items.length === 0) && (
              <div className="py-12 text-center text-muted-foreground text-sm">
                {t('staff:noItemsOrdered', { defaultValue: 'No items ordered yet. Click Add Items above.' })}
              </div>
            )}
          </div>
        </div>
      ...
      <Sheet open={catalogOpen} onOpenChange={setCatalogOpen}>
        <SheetContent side="right" className="w-full max-w-lg p-0 flex flex-col h-full bg-card border-l border-border" showCloseButton={false}>
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {filteredItems.map((item) => {
              ...
              return (
                <div key={item.id} className="p-3 bg-background border border-border rounded-xl flex justify-between items-center gap-4">
                  <div className="min-w-0">
                    <h4 className="text-xs font-bold text-primary truncate">{name}</h4>
                    <CurrencyDisplay amount={item.price} className="text-xs text-muted-foreground font-semibold" />
                  </div>
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      <div className="flex items-center gap-1.5 bg-card border border-border rounded-lg p-0.5">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => updateLocalCartQty(item.id, inCart.quantity - 1)}
                          className="w-6 h-6 rounded bg-muted text-foreground flex items-center justify-center text-xs cursor-pointer"
                        >
                          -
                        </Button>
                        <span className="text-xs font-black text-foreground w-4 text-center">{inCart.quantity}</span>
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => updateLocalCartQty(item.id, inCart.quantity + 1)}
                      ...
                      <Input
                        type="text"
                        value={inCart.note}
                        onChange={(e) => updateLocalCartNote(item.id, e.target.value)}
          ...
          <div className="p-4 border-t border-border bg-card space-y-4">
            <div className="flex justify-between items-center text-xs font-bold text-foreground">
              <span>{t('staff:selectedCount', { count: Object.values(localCart).reduce((acc, i) => acc + i.quantity, 0), defaultValue: `Selected (${Object.values(localCart).reduce((acc, i) => acc + i.quantity, 0)}):` })}</span>
              <CurrencyDisplay
                amount={Object.values(localCart).reduce((acc, i) => acc + i.price * i.quantity, 0)}
                className="text-primary font-extrabold"
              />
            </div>
            <Button
              onClick={handleAddItemsSubmit}
              disabled={addItemsMutation.isPending || Object.keys(localCart).length === 0}
