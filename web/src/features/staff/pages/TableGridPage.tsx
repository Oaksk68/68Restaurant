...
export default function TableGridPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-xl font-black text-primary">{t('staff:tableManagement')}</h1>
        <p className="text-xs text-muted-foreground mt-1">
          {tables.filter(t => t.status === 'available').length} {t('common:available')} &bull;{' '}
          {tables.filter(t => t.status === 'occupied').length} {t('common:occupied')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">
        {tables.map((table) => {
          ...
          let badgeColorClass = 'border-emerald-500/35 bg-emerald-500/5 text-emerald-400'
          
          if (isOccupied) {
            if (isBilled) {
              statusText = t('common:billed')
              badgeColorClass = 'border-amber-500/35 bg-amber-500/5 text-amber-400'
            } else {
          ...
          const itemsCount = activeOrder?.items?.reduce((acc, i) => acc + i.quantity, 0) || 0
          const itemsTotal = activeOrder?.items?.reduce((acc, i) => acc + i.unit_price * i.quantity, 0) || 0

          return (
            <button
              key={table.id}
              onClick={() => navigate(`/staff/tables/${table.id}`)}
              className={`glass flex flex-col justify-between text-left p-4 rounded-2xl border transition-all duration-200 hover:scale-[1.02] group cursor-pointer ${isOccupied
                ? isBilled
              ...
              <div className="flex justify-between items-start w-full">
                <div>
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-muted-foreground mt-0.5">
                    <Users size={10} />
                    <span>{table.capacity} {t('staff:seats', { defaultValue: 'Seats' })}</span>
                  </div>
                </div>
              ...
              {isOccupied && activeOrder ? (
                <div className="mt-6 pt-2 border-t border-border space-y-1 w-full">
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                    ...
                    {activeOrder.note && <span className="text-amber-400">* Note</span>}
                  </div>
